...
F-00.1 and the F-00.3 local recording spike are in progress. The repository contains a new Rust/Postgres API, a Flutter signed-in shell, shared v1 wire examples, local database Compose configuration, and Android/macOS CI definitions. Google sign-in → account disclosure → manually authored text memory → own-library Ask → deletion has been exercised on a physical Android phone against the isolated local backend. A separate [voice-draft spike](docs/VOICE_DRAFT_SPIKE.md) can capture temporary audio locally, promote completed drafts from cache into protected temporary storage, reconcile them on restart, expire them and delete them; it does not upload, transcribe or publish a memory. Actual microphone recording on a physical phone, Apple sign-in and iOS device behavior remain unverified. Offline sync and reciprocal-friend access are not implemented yet. No delivery phase has passed its full gate.

Continue F-00.3 physical-device recording/backup checks and F-00.2 design review, then connect F-01.1 backend transcription with separate consent and accepted-audio deletion. The first full product outcome remains signed-in voice/text capture → private transcript → useful organized knowledge → own Ask, with prompt audio deletion and explicit source controls. Production text/URL share input arrives in early F-02; advanced capture polish remains F-06. Proposed limits and quality targets still require measurement.
