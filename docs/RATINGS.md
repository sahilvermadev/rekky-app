...
Decision: 2026-09-27. Automatic ratings describe an author's opinion, not an external review average. The existing `gpt-4.1-mini` understanding request proposes the score alongside extraction. There is no additional request or interactive capture field. The expanded sheet shows it near the category and before locality/Maps, and Library cards show a compact score alongside caution/review signals; an inferred rating has a small **Estimated** marker with accessible explanatory text. Owners can change or remove it in Edit recommendation.
...
## Card scores and requested backfill, 2026-09-27

Ratings now appear on Library cards as a small star, score out of ten and Estimated marker when inferred. The same metadata row accommodates caution/review signals, wraps at large text sizes and adds no summary or placeholder for unrated notes. The bottom sheet retains the fuller accessible explanation. Flutter analysis and all 38 affected rating/card/detail tests passed, including 320-pixel layouts at 200% text. The Android APK was rebuilt and installed; all three scores were visually verified on the connected phone’s Library cards.

The owner explicitly approved one model request for each of three existing recommendations. The `rate_existing` maintenance example prepares a private local proposal for one named item/revision, then applies only its validated rating in a separate transaction. It requires a single live item in the capture, unchanged source and item revisions, live extraction permission/generation, unsuperseded support and no owner edits or existing score. It never scans accounts or runs on opening a card; it uses one request per preparation, without automatic retries. Evidence is retained privately in source support and cascades with source deletion. This operator-invoked maintenance path does not use the normal queued-job budget ledger and is not a general product backfill API.

All three requested ratings were added once. Before/after hashes verified unchanged subject/body, non-rating recommendation fields, audience, original transcript and source revision. No real source text or private proposal files are committed to this repository. Initial-build statements above about no historical reprocessing describe the earlier checkpoint; this explicitly authorized backfill is the subsequent exception.
